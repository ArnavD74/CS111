public class LeasingCost {
    ...
    public static Vehicle createVehicle(String description) {
        ...
        startIndex = description.indexOf(":", description.indexOf("type")) + 1;
        endIndex = description.indexOf(";", startIndex);


        System.out.println(description);

        type = description.substring(startIndex, endIndex);

        if (type.equals("electric")) {
            startIndex = description.indexOf(":", description.indexOf("charger"))  + 1;
            endIndex = description.indexOf(";", startIndex);
            charger = Double.parseDouble(description.substring(startIndex, endIndex));
        }

        startIndex = description.indexOf(":", description.indexOf("name")) + 1;
        endIndex = description.indexOf(";", startIndex + 1);
        name = description.substring(startIndex, endIndex);

        startIndex = description.indexOf(":", description.indexOf("due")) + 1;
        endIndex = description.indexOf(";", startIndex + 1);
        due = Double.parseDouble(description.substring(startIndex, endIndex));

        startIndex = description.indexOf(":", description.indexOf("length"))  + 1;
        endIndex = description.indexOf(";", startIndex + 1);
        length = Integer.parseInt(description.substring(startIndex, endIndex));

        startIndex = description.indexOf(":", description.indexOf("monthly"))  + 1;
        endIndex = description.indexOf(";", startIndex + 1);
        monthly = Double.parseDouble(description.substring(startIndex, endIndex));

        startIndex = description.indexOf(":", description.indexOf("mile/unit"))  + 1;
        endIndex = description.indexOf(";", startIndex + 1);
        usage = Double.parseDouble(description.substring(startIndex, endIndex));

        startIndex = description.indexOf(":", description.indexOf("allowance"))  + 1;
        endIndex = description.indexOf(";", startIndex + 1);
        allowance = Integer.parseInt(description.substring(startIndex, endIndex));

        //public Fuel (double usage, double charger) {
        Fuel fCharger = new Fuel(usage, charger);
        Fuel fNoncharger = new Fuel(usage);

        //public Lease (double dueAtSigning, int leaseLength, double monthlyCost,int mileageAllowance) {
        Lease l = new Lease(due, length, monthly, allowance);

        Vehicle v;

        if (charger > 0) {
            v = new Vehicle(name, fCharger, l);
        } else {
            v = new Vehicle(name, fNoncharger, l);
    ...
    public static void computeCO2EmissionsAndCost(Vehicle[] vehicles, double gasPrice, double electricityPrice) {
        ...
        for (int i = 0; i < vehicles.length; i++) {
            ...
            if (vehicles[i].getFuel().getCharger() != 0.0) {
                ...
                co2usage = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), electricityPrice);
            } else {
                ...
                co2usage = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), gasPrice);
            }
            ...
            vehicles[i].setCO2Emission(co2usage);

            if (vehicles[i].getFuel().getCharger() != 0.0) {
                double charger = vehicles[i].getFuel().getCharger();
                vehicles[i].setTotalCost(fuelCost + leaseCost + charger);
            } else {
                vehicles[i].setTotalCost(fuelCost + leaseCost);
            }
